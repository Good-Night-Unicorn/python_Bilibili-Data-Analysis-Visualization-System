		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.author" placeholder="up主" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">类型：</div>
					<el-input v-model="formSearch.typename" placeholder="类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('hotvideo','新增')" type="primary" @click="add('/index/hotvideoAdd')">
			...
			<div class="list">
				...
				<div class="list2 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" class="list-item animation-box" @click.stop="toDetail(item)">
						<div class="img">
							<img @click.stop="imgPreView(item.cover)" v-if="item.cover && item.cover.substr(0,4)=='http'&&item.cover.split(',w').length>1" :src="item.cover" class="image" />
							<img @click.stop="imgPreView(item.cover.split(',')[0])" v-else-if="item.cover && item.cover.substr(0,4)=='http'" :src="item.cover.split(',')[0]" class="image" />
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.author != '') searchWhere.author = '%' + this.formSearch.author + '%';
				if (this.formSearch.typename != '') searchWhere.typename = '%' + this.formSearch.typename + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`hotvideo/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/hotvideoDetail', query: params});
			},
