	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/hotvideo?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.info}}</div>
					</div>
					<div class="item">
						<div class="lable">时长（秒）</div>
						<div class="text "  >{{detail.duration}}</div>
					</div>
					<div class="item">
						<div class="lable">up主</div>
						<div class="text "  >{{detail.author}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">弹幕量</div>
						<div class="text "  >{{detail.danmaku}}</div>
					</div>
					<div class="item">
						<div class="lable">点赞</div>
						<div class="text "  >{{detail.likes}}</div>
					</div>
					<div class="item">
						<div class="lable">收藏</div>
						<div class="text "  >{{detail.favorite}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.commentcount}}</div>
					</div>
					<div class="item">
						<div class="lable">上传地点</div>
						<div class="text "  >{{detail.publocation}}</div>
					</div>
					<div class="item">
						<div class="lable">上传时间</div>
						<div class="text "  >{{detail.pubtime}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">投币</div>
						<div class="text "  >{{detail.coin}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('hotvideo','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('hotvideo','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.cover) {
							this.detailBanner = this.detail.cover.split(",w").length>1?[this.detail.cover]:this.detail.cover.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('hotvideo/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'hotvideo', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/hotvideo', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discusshotvideo/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusshotvideo/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discusshotvideo/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discusshotvideo/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/hotvideoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此热门视频？') .then(_ => {
					this.$http.post('hotvideo/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
